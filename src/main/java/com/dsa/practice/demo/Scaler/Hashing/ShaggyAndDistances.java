...
public class ShaggyAndDistances {
    ...
    public int findMinimumDistanceForSpecialPair(int[] A) {
        HashMap<Integer, Integer> indexMap = new HashMap<>();
        int minimumDistance = Integer.MAX_VALUE;

        for (int i = 0; i < A.length; i++) {
            if (indexMap.containsKey(A[i])) {
                // Calculate the distance between the current index and the index of the same element.
                int currentDistance = i - indexMap.get(A[i]);
        ...
        if (minimumDistance == Integer.MAX_VALUE) {
            return -1; // If no special pair exists, return -1.
        }
