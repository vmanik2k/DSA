package com.dsa.practice.demo.Scaler.Hashing;
import java.util.HashSet;
/*Given an Array of integers B, and a target sum A.
Check if there exists a pair (i,j) such that Bi + Bj = A and i!=j.


Problem Constraints
1<= Length of array B <= 103
0<= Bi <=109
0<= A <=109


Input Format
First argument A is the Target sum, and second argument is the array B


Output Format
Return an integer value 1 if there exists such pair, else return 0.


Example Input
Input 1:

A = 8
B = [3, 5, 1, 2, 1, 2]
Input 2:

A = 21
B = [9, 10, 7, 10, 9, 1, 5, 1, 5]


Example Output
Output 1:

1
Output 2:

0


Example Explanation
Example 1:
It is possible to obtain sum 8 using 3 and 5.
Example 2:
There is no such pair exists.*/
public class CheckPairSum {
        // This function checks if there is a pair of elements in the array 'B' whose sum equals 'A'.
        public int checkPairSum(int targetSum, int[] array) {
            // Create a HashSet to store elements from the 'B' array.
            HashSet<Integer> elementsSet = new HashSet<>();

            // Iterate through the 'B' array.
            for (int i = 0; i < array.length; i++) {
                int complement = targetSum - array[i]; // Calculate the complement of the current element.

                // If the complement is already in the HashSet, we found a pair whose sum equals 'A'.
                if (elementsSet.contains(complement)) {
                    return 1;
                }

                // Add the current element to the HashSet for future reference.
                elementsSet.add(array[i]);
            }

            // If no such pair is found, return 0.
            return 0;
        }

    public static void main(String[] args) {
        System.out.println(new CheckPairSum().checkPairSum(8,new int[]{3, 5, 1, 2, 1, 2}));
    }


}
